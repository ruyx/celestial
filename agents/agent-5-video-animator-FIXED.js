...
/**
 * AGENTE 5: VIDEO ANIMATOR (FIXED - Sin duplicados)
 *
 ...
 * Usa Kling 1.6 Pro que soporta hasta 30s
 *
...
async function generateVideoBatch(imageMetadataPath) {
  console.log('🎬 Agente 5: Video Animator (FIXED)');
  console.log('====================================\n');
  ...
  for (const image of imageMetadata.scenes) {
    ...
    // Limitar duración a 30s (máximo de Kling 1.6 Pro)
    const videoDuration = Math.min(duration, 30);

    if (duration > 30) {
      console.log(`   ⚠️  Duración original ${duration}s excede límite, reducida a 30s`);
    }
  ...
  const specFile = `video-${imageMetadata.verse.replace(/[:\s]/g, '-')}-FIXED-${Date.now()}.json`;
  ...
  console.log(`   Modelo: Kling 1.6 Pro (hasta 30s por video)`);
