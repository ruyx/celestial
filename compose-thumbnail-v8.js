#!/usr/bin/env node
/**
 * 🎨 THUMBNAIL COMPOSER V8 - DINÁMICO Y RESPONSIVE
 *
 * Cambios críticos vs V7:
 * ✅ NO hardcoded - calcula tamaños dinámicamente
 * ✅ Ajusta automáticamente si el texto es muy largo
 * ✅ Interlineado calculado (1.15x altura del texto)
 * ✅ Respeta límites del canvas (1344x768)
 * ✅ Funciona con textos cortos y largos
 *
 * Algoritmo:
 * 1. Medir texto con ImageMagick
 * 2. Si excede 40% ancho (538px), reducir tamaño
 * 3. Calcular interlineado = altura × 1.15
 * 4. Centrar bloque verticalmente
 */
...
function measureTextWidth(text, font, size) {
  try {
    const cmd = `convert -debug annotate xc: \\
      -font ${font} \\
      -pointsize ${size} \\
      -annotate +0+0 "${text}" \\
      null: 2>&1 | grep "Metrics:" | head -1`;

    const output = execSync(cmd, { encoding: 'utf8' });

    // Parse: Metrics: text: 640x120+0+0
...
async function composeThumbnail(baseImagePath, verse, text, category, outputPath) {
  ...
  try {
    ...
    console.log(`✅ Thumbnail V8 generado exitosamente`);
...
if (require.main === module) {
  ...
  if (args.length < 5) {
    console.error('\n❌ Uso: node compose-thumbnail-v8.js <base-image> <verse> <text> <category> <output>');
    console.error('\nEjemplo:');
    console.error('  node compose-thumbnail-v8.js base.png "Proverbios 3:5-6" "Confía en Dios SIEMPRE" sabiduria output.jpg\n');
    process.exit(1);
  }
